...
function TBag_ReagentHelper_ModifyItemTooltip(bag, slot, tooltipName, itm)
...
function TBag_LootLink_SetTooltipMoney(frame, count, money, stack)
...
function TBag_LootLink_ModifyItemTooltip(bag, slot, tooltipName, itm)
  ...
  if ( ( InRepairMode() ) or
    ( TBag_ModifyTooltipsAtMerchant == 0 ) and ( MerchantFrame:IsVisible() ) ) then
    shouldModify = false;
  end
  if ( 
    ...
    if ( ( not LootLinkState ) or ( not LootLinkState.HideInfo ) ) and ( name ) and ( data ) then
      if ( itm ) and ( itm["itemname"] ) then
        if ( TBag_ShowPrice == 1 ) then
          ...
          if ( money ) then
            TBag_LootLink_SetTooltipMoney(tooltip, itm["itemcount"], money, stack);
          end
        end
...
function TBag_RegisterCurrentTooltipSellValue(tooltip, bag, slot, itm)
...
--
-- Packrat:  If you want me to stop calling you, set TBag_DontCallPackrat to anything.
--
function TBag_ModifyItemTooltip(bag, slot, tooltipName, itm)
  if ( not tooltipName ) then
    tooltipName = "GameTooltip";
  end

  if ( (PackRat ~= nil) and (TBag_DontCallPackrat == nil) ) then
    PackRat._itemEntered = { bag, slot };
    PackRat:DisplayTooltipPrice()
  end

  TBag_ReagentHelper_ModifyItemTooltip(bag, slot, tooltipName, itm);
  TBag_LootLink_ModifyItemTooltip(bag, slot, tooltipName, itm);
  local tooltip = getglobal(tooltipName);
  if ( not tooltip ) then  
    return;
  end
