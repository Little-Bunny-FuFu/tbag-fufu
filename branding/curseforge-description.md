...
![Auto-sorted inventory](REPLACE_WITH_URL)
*The unified, auto-sorted inventory window.*
...
![Item search](REPLACE_WITH_URL)
*Search across the current window and your cached data.*

![Manual layout mode](REPLACE_WITH_URL)
...
![Cross-character viewing](REPLACE_WITH_URL)
*Browse a cached alt's bags and bank from the name dropdown.*

### Modern 12.0 bank (built on the new `C_Bank` API)

![Warband bank](REPLACE_WITH_URL)
*Character/Warband bank with the per-tab selector strip and deposit-all buttons.*
...
![Bank tab settings](REPLACE_WITH_URL)
*Right-click a tab: rename, icon, auto-deposit flags, and expansion filter.*

### Configuration
- Extensive in-game options, with **separate inventory and bank option panels**,
  plus an advanced configuration window.

![Options panel](REPLACE_WITH_URL)
*Separate inventory and bank option panels.*
